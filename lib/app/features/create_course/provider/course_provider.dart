...
import 'package:spot_on/app/features/create_course/repository/course_repository.dart';
...
class CourseNotifier extends StateNotifier<CourseState> {
  ...
  Future<void> getCourses(String institutionId) async {
    state = const CourseState.loading();
    final result = await _repository.getCourses(institutionId).run();
    result.fold(
      (l) {
        log(l);
        state = CourseState.error(l);
      },
      (r) => state = CourseState.data(r.items),
    );
  }
}
